...
export function useReceiptUpload(): UseReceiptUploadReturn {
  ...
  const deleteReceipt = useCallback(async (userId: string, transactionId: string): Promise<void> => {
    setIsUploading(true);
    setUploadError(null);
  ...
  const getReceiptUrl = useCallback(async (userId: string, transactionId: string): Promise<string | null> => {
    try {
